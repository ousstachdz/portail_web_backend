...
class Lead(models.Model):
    ...
    def _search(self, domain, offset=0, limit=None, order=None, access_rights_uid=None):
        _logger.info("_______________________ start _search")
        user = self.env.user
        if access_rights_uid:
            user = self.env['res.users'].browse(access_rights_uid)
        
        domain = self._apply_branch_security(domain, user)
        return super(Lead, self)._search(domain, offset=offset, limit=limit, order=order, access_rights_uid=access_rights_uid)


    @api.model
    def search_read(self, domain=None, fields=None, offset=0, limit=None, order=None):
        _logger.info("_______________________ start search_read")

        domain = domain or []
        
        # Don't apply security for special cases like creation
        if self._context.get('check_field_access_rights'):
            return super(Lead, self).search_read(domain=domain, fields=fields, offset=offset, limit=limit, order=order)
        
        domain = self._apply_branch_security(domain, self.env.user)
        return super(Lead, self).search_read(domain=domain, fields=fields, offset=offset, limit=limit, order=order)


    def _apply_branch_security(self, domain, user):
        _logger.info("Apply branch-based security")
        _logger.info(f"Original domain: {domain}")
        _logger.info(f"User: {user.name}, Branch: {user.branche.name if user.branche else 'None'}")

        # Get the actual XML IDs from your debug output and use them here
        is_admin = user.has_group('base.group_system') or user.has_group('dept_comm.dept_comm_group_administration')
        is_responsable = user.has_group('dept_wk.dept_wk_group_responsable_agence')
        is_agent = user.has_group('dept_wk.dept_wk_group_agent_agence')

        _logger.info(f"Groups - Admin: {is_admin}, Responsable: {is_responsable}, Agent: {is_agent}")

        # Administration → full access
        if is_admin:
            _logger.info("User is admin - full access")
            return domain

        # Agents + Responsable → restrict by agence
        if is_responsable or is_agent:
            user_branch = user.branche.id if user.branche else False

            _logger.info(f"User branch ID: {user_branch}")

            if user_branch:
                branch_domain = [('branche', '=', user_branch)]
            else:
                # If user has no branch, they shouldn't see any records
                branch_domain = [('id', '=', False)]  # This returns empty recordset

            _logger.info(f"Branch domain: {branch_domain}")

            # Merge domains
            if domain:
                new_domain = expression.AND([domain, branch_domain])
            else:
                new_domain = branch_domain

            _logger.info(f"Final domain: {new_domain}")
            return new_domain

        _logger.info("User doesn't have restricted groups - full access")
        return domain
    ...
    display_name = fields.Char(
        string='oppertunity name', compute='_compute_display_name')
    ...
    groupe = fields.Many2one('res.partner', string='المجموعة',domain=[('is_duplicate_partner','=', False)])
    classification = fields.Many2one(
        'wk.classification', string="تصنيف الشركة")
    adress_siege = fields.Char(string='عنوان المقر الاجتماعي')
    wilaya = fields.Many2one(
        'wk.wilaya', string='الولاية', related='branche.wilaya_id')
    ...
    activite = fields.Many2one(
        'wk.activite', string='النشاط الرئيسي حسب بنك الجزائر')
    activite_salam = fields.Many2one(
        'wk.activite.salam', string='النشاط الرئيسي حسب بنك السلام')
    activite_second = fields.Many2one(
        'wk.secteur', string='رمز النشاط الثانوي في السجل التجاري')
    ...
    apropos = fields.One2many(
        'wk.partenaire', 'lead_id', string='توزيع راس مال الشركة')
    gestion = fields.One2many('wk.gestion', 'lead_id', string='فريق التسيير')
    tailles = fields.One2many('wk.taille', 'lead_id',
                              string='حجم و هيكل التمويلات المطلوبة')
    fournisseurs = fields.One2many(
        'wk.fournisseur', 'lead_id', string='الموردون')
    clients = fields.One2many('wk.client', 'lead_id', string='الزبائن')
    situations = fields.One2many(
        'wk.situation', 'lead_id', string='الوضعية المالية')
    situations_fin = fields.One2many(
        'wk.situation.fin', 'lead_id', string='البيانات المالية المدققة للثلاث سنوات الأخيرة KDA')
    kyc = fields.One2many('wk.kyc.details', 'lead_id', string='اعرف عميلك KYC')
    companies = fields.One2many(
        'wk.companies', 'lead_id', string='معلومات حول الشركات ذات الصلة (KDA)')
    documents = fields.One2many(
        'wk.document.check', 'lead_id', string='الملفات')
    ...
    def convert_dossier(self):
        for rec in self:
            if rec.partner_id.id== False:
                raise UserError('لا يمكن أن يكون حقل اسم العميل فارغًا')
            if not rec.demande:
                raise UserError('حقل الطلب لا يمكن أن يكون فارغًا')
            ...
            final_stage = self.env['crm.stage'].sudo().search([('is_won', '=', True)])
            rec.stage_id = final_stage.id
            self.is_convert_dossier = True
            ...
            return {
                ...
                'params': {
                    'title': _('Success'),
                    'message': _('Workflow Dashboard created successfully!'),
                    'type': 'success',  
                    'sticky': False,    
                }
            }
