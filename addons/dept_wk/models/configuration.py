...
class WKClientCompte(models.Model):
    _name = 'wk.compte.client'
    
    etape_id = fields.Many2one('wk.consulter.compte')
    account_id = fields.Char(string="Account ID")
    ledgerBalance = fields.Float(string="Ledger Balance")
    workingBalance = fields.Float(string="Working Balance")
    accountCurrency = fields.Char(string="Currency")
    categoryName = fields.Char(string="Category")
...
class Signature(models.Model):
    ...
    multi_signature_id=fields.Many2one('wk.multi.signature')
    multi_signature_id_wizard=fields.Many2one('wk.multi.signature.wizard')
    position=fields.Char(string='المنصب')
    position_comite = fields.Selection(
    string='المنصب في اللجنة',
    selection=[
...
class MultiSignature(models.Model):
    _name='wk.multi.signature'
    _description='multi-signatures'
    
    
    reference=fields.Char('')
    signature_ids=fields.One2many('wk.signature',"multi_signature_id",string="التوقيعات المرفقة")
    
    def make_it_default_signature(self):
        self.env['ir.config_parameter'].sudo().set_param('wk.default_signature', self.id)
