...
class Partner(models.Model):
    ...
    # id_customer = fields.Integer(string="رقم العميل")
    ...
    def get_customer_info(self):
        ...
        url = f"https://10.116.70.27:9443/asbaws/api/v1.0.0/holdings/customers/{self.id_customer}"
        _logger.info("Calling get_customer_info for id_customer=%s", self.id_customer)
        try:
            headers = {
            ...
            if "body" not in data or not isinstance(data["body"], list) or not data["body"]:
                _logger.warning('No account data found for account number: %s', self.id_customer)
                raise UserError(_("No account data found for the given account number."))
            account_data = data["body"][0]
            target_value = account_data.get('target', '')
            accountOfficerID_value = account_data.get('accountOfficerId', '')
            if accountOfficerID_value:
                accountOfficerID_value = '0' + accountOfficerID_value
            branch_obj = self.env['wk.agence']
            branch_record = branch_obj.search([('name', '=', accountOfficerID_value)], limit=1)
            if not branch_record:
                branch_record = branch_obj.create({'name': accountOfficerID_value})
                
            classification_obj = self.env['wk.classification']
            classification_record = classification_obj.search([('name', '=', target_value)], limit=1)
            if not classification_record:
                classification_record = classification_obj.create({'name': target_value})

            self.write({
                'id_customer': account_data.get('customerId', ''),
                'adress_siege': account_data.get('customerAddress', ''),
                'name': account_data.get('customerShortName', ''),
                'phone': account_data.get('smsNumber', ''),
                'email': account_data.get('email', ''),
                'classification': classification_record.id, 
                'branche': branch_record.id,
                # '': account_data.get('partyType', ''),
                # '': account_data.get('mnemonic', ''),
    ...
    def get_customer_info(self):
        token = self.authenticate()
        url = f"https://10.116.70.27:9443/asbaws/api/v1.0.0/holdings/customers/{self.customer_id}"
        _logger.info("Calling get_customer_info for customer_id=%s", self.customer_id)
        try:
            headers = {
                'Content-Type': 'application/json',
                "Authorization": f"Bearer {token}"
            ...
            data = response.json()
            if "body" not in data or not isinstance(data["body"], list) or not data["body"]:
                _logger.warning('No account data found for account number: %s', self.customer_id)
                raise UserError(_("No account data found for the given account number."))
            account_data = data["body"][0]
            self.write({
                'id_customer': account_data.get('customerId', ''),
                ...
                'phone': account_data.get('smsNumber', ''),
                'email': account_data.get('email', ''),
                # '': account_data.get('target', ''),
                # '': account_data.get('partyType', ''),
                # '': account_data.get('mnemonic', ''),
                # '': account_data.get('accountOfficerId', ''),
            })
        except requests.exceptions.RequestException as e:
            raise UserError(_("An error occurred while fetching data: %s") % str(e))
        except Exception as e:
    ...
    workflows_linked = fields.One2many('wk.workflow.dashboard', 'nom_client', string='Workflows')
    
    @api.depends('workflows_linked')
    def compute_workflow_count(self):
        for rec in self:
            rec.workflow_count = len(rec.workflows_linked)
