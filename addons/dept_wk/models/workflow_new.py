...
class Workflow(models.Model):
    ...
    state = fields.Selection([('1', 'الفرع'),
                              ...
                              ('4', 'ادارة المخاطر'),
                              ('16','رئيس قطاع التمويلات'),
                              ('17','رئيس قطاع المالية'),                              
    ...
    signatures = fields.Json(string='Signatures Data')
    ...
    comite_type=fields.Char("comite type", compute='compute_comite_type',store=False)
    ...
    state_finance_related = fields.Selection([
    ...
    @api.onchange('state')
    def _onchange_rec_state(self):
        if self.state == "2":
           self.state_finance_related ="finance_1"
    
    
    
    def compute_comite_type(self):
        for rec in self:
            if rec.states.filtered(lambda l:l.sequence == 2):
                rec.comite_type = rec.states.filtered(lambda l:l.sequence == 2).comite.display_name
            else:
                rec.comite_type = ''  
    ...
    def generate_signature_tables(self):
        """
            Generates a structured list of signature tables for the report.

            Process:
            - Retrieves signatures from the `wk.signature` model.
            - Filters out signatures where `position_comite` is 'رئيس'.
            - Groups the remaining signatures into tables, each containing up to 2 rows.
            - Ensures proper handling of odd and even numbers of signatures.

            Returns:
            - slices_table: A list of tables, where each table consists of up to 2 signatures.

        """
        
       
        if not self.env['ir.config_parameter'].sudo().get_param('wk.default_signature'):
            multi_signature = self.env.ref('dept_wk.multi_signature_1', raise_if_not_found=False)
            self.env['ir.config_parameter'].sudo().set_param('wk.default_signature', multi_signature.id)
        ...
        default_signatures = self.env["wk.multi.signature"].browse(default_signatures_id).signature_ids
        self.signatures = [
        ...
        signatures_table=self.signatures
        slices_table = []
        signatures_table = list(filter(lambda l: l['position_comite'] != 'رئيس اللجنة',signatures_table))
        tables_count = int(signatures_table.__len__()/2)
        
        
        if is_odd(signatures_table.__len__()):  
            for table_index in range(0,tables_count):
                row = signatures_table[0+(table_index*2):2+(table_index*2)]
                slices_table.append(row)
           
            
        else:
            for table_index in range(0,tables_count+1):
                row = signatures_table[0+(table_index*2):2+(table_index*2)]
                slices_table.append(row)
    ...
    def get_president_signature(self):
        """
            Retrieves the signature details of the president.

            Process:
            - Iterates through the list of signatures.
            - Finds the record where `position_comite` is 'رئيس' (President).
            - Stores the president's username and position in a dictionary.

            Returns:
            - A dictionary containing the president's `username` and `position`. 
            Returns an empty dictionary if no president is found.
        ...
        signatures_table=default_signatures
        signatures_table = list(filter(lambda l: l['position_comite'] == 'رئيس اللجنة',signatures_table))
        president_signature={}
        for rec in signatures_table:
                president_signature['position_comite'] = rec["position_comite"]
                president_signature['position'] = rec["position"]
        return president_signature
    ...
    def action_start(self):
        for rec in self:
            ...
            elif rec.demande in [self.env.ref('dept_wk.type_demande_2')] and not rec.workflow_old:
                etape = self.env['wk.etape'].create({'workflow': rec.id,
                                                     'etape': self.env.ref('dept_wk.princip_1').id,
                                                     'state_branch': 'branch_1'})
            elif rec.demande in [self.env.ref('dept_wk.type_demande_2')] and rec.workflow_old:
            ...
            elif rec.demande in [self.env.ref('dept_wk.type_demande_3'),self.env.ref('dept_wk.type_demande_4')] and rec.workflow_old:
                for etape in rec.workflow_old.states:
                    if etape.sequence == 2:
                        self.state_finance_related ="finance_1"
                    vals = get_values(rec, etape)
...
def get_values(workflow, etape):
    ...
    if etape.sequence == 1:
        values = {
            ...
            'tcr_situation_estim': etape.tcr_situation_estim.id,
            'file_tcr_estim': etape.file_tcr_estim,
            'file_tcr_name_estim': etape.file_tcr_name_estim,
            'actif_situation': etape.actif_situation.id,
            'passif_situation': etape.passif_situation.id,
            'tcr_situation': etape.tcr_situation.id,
            'file_tcr': etape.file_tcr,
            'file_tcr_name': etape.file_tcr_name,
            'required_funds': etape.required_funds,
            'kda': etape.kda,
        }
    ...
    'garantie_actuel_comment': etape.garantie_actuel_comment,
    'garantie_propose_comment': etape.garantie_propose_comment,
    'comment_risk_central': etape.comment_risk_central,
    'detail_mouvement': etape.detail_mouvement,
    'comment_fisc': etape.comment_fisc,
    'comment_bank': etape.comment_bank,
    'comment_salam_bank': etape.comment_salam_bank,
    ...
    'commentaire_situation': etape.commentaire_situation,
    ...
    #         'file_tcr': etape.file_tcr,
    #         'file_tcr_name': etape.file_tcr_name,
    #         'apropos': etape.apropos,
    #         'kyc': etape.kyc,
    #         'management_team_file1': etape.management_team_file1,
    #         'gestion': etape.gestion,
    #         'employees': etape.employees,
    #         'headquarter': etape.headquarter,
    #         'sieges': etape.sieges,
    #         'required_funds': etape.required_funds,
    #         'tailles': etape.tailles,
    #         'garanties': etape.garanties,
    #         'financial_situation_file1': etape.financial_situation_file1,
    #         'situations': etape.situations,
    #         'annee_fiscal_list': etape.annee_fiscal_list.id,
    #         'situations_fin': etape.situations_fin,
    #         'suppliers_clients_file1': etape.suppliers_clients_file1,
    #         'fournisseur': etape.fournisseur,
    #         'suppliers_clients_file2': etape.suppliers_clients_file2,
    #         'client': etape.client,
    #         'kda': etape.kda,
    #         'companies': etape.companies
    #     }
