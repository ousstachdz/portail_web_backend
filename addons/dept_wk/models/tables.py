...
class DetailGarantiePropose(models.Model):
    ...
    montant_dollar = fields.Float(string='القيمة (بالدولار)')
...
class BilanCateg1(models.Model):
    ...
    year_1_display = fields.Char(string='N-3', compute='compute_display_years')
    year_2_display = fields.Char(string='N-2', compute='compute_display_years')
    year_3_display = fields.Char(string='N-1', compute='compute_display_years')
    year_4_display = fields.Char(string='N', compute='compute_display_years')

    def compute_display_years(self):
        for rec in self:
            if rec.declaration == 'السنة':
                rec.year_1_display = int(rec.year_1 or 0.0)
                rec.year_2_display = int(rec.year_2 or 0.0)
                rec.year_3_display = int(rec.year_3 or 0.0)
                rec.year_4_display = int(rec.year_4 or 0.0)
            else:
                rec.year_1_display = rec.year_1
                rec.year_2_display = rec.year_2
                rec.year_3_display = rec.year_3
                rec.year_4_display = rec.year_4
