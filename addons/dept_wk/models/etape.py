...
class Etape(models.Model):
    ...
    ### Avis pour le nouveau recours
    stab_avis_agence = fields.Html(string="توصية مدير الفرع من اجل طلب الطعن ")
    stab_avis_analyse = fields.Html(string="توصية مدير ادارة التمويلات من اجل طلب الطعن")
    stab_avis_commerciale = fields.Html(string="توصية مدير إدارة الاعمال التجارية من اجل طلب الطعن")
    stab_avis_risque = fields.Html(string="توصية مدير ادارة المخاطر من اجل طلب الطعن")
    ...
    excel_file = fields.Binary(string='الملف الاكسيل')
    ...
    resume_activite = fields.Text('نبذة عن نشاط الشركة' )
    ...
    def update_state_finance_related(self):
        if self.etape.sequence == 2:
           self.workflow.state_finance_related = self.state_finance
    ...
    risque_ids = fields.One2many('risk.scoring', 'etape_id', string='الشركات ذات الصلة',
                                domain="[('is_initial', '=', False)]", compute="_compute_companies_linked", store="True")
    
    compute_companies_linked_executed = fields.Boolean(default=False)
    prev_client_nom = fields.Char()
    @api.depends()
    def compute_companies_linked(self):
        linked_records=[]
        if self.compute_companies_linked_executed == False or self.prev_client_nom != self.nom_client.name:
            for record in self:
                record.risque_ids = [(5, 0, 0)]
                workflow_found = False
                client_group = record.workflow.nom_client.groupe
                if client_group:
                    workflows_state_7 = self.env['wk.workflow.dashboard'].search([
                            ('state', '=', '7')
                        ])
                    related_partners = self.env['res.partner'].search([
                            ('groupe', '=', client_group.id),
                            ('id', '!=', record.workflow.nom_client.id),
                            ('id', 'in', workflows_state_7.mapped('nom_client').ids)
                        ])
                    for partner in related_partners:
                        workflows = self.env['wk.workflow.dashboard'].search([
                            ('state', '=', '7'),
                            ('nom_client', '=', partner.id)
                        ])
                        
                        for workflow in workflows:
                            wk_states = workflow.states.filtered(lambda l: l.etape.sequence == 2)
                            for wk in wk_states:
                                if wk.tcr_id and wk.actif_id and wk.passif_id and workflow_found == False:
                                    workflow_found = True
                                    new_tcr = wk.tcr_id.copy()
                                    new_tcr.parent_id=False
                                    new_passif = wk.passif_id.copy()
                                    new_passif.parent_id=False
                                    new_actif = wk.actif_id.copy()
                                    new_actif.parent_id=False
                                    linked_records = [(0, 0, {
                                        'partner_id': partner.id,
                                        'date': date.today(),
                                        'is_initial': False,
                                        'tcr_id': wk.tcr_id.id,
                                        'actif_id': wk.actif_id.id,
                                        'passif_id': wk.passif_id.id
                                })]
                    if linked_records:                            
                        record.risque_ids = linked_records
                    self.risque_ids.import_data_group_alternative()
                self.compute_companies_linked_executed = True
                self.prev_client_nom = self.nom_client.name    
    ...
    # President Fondement
    state_pre_fond = fields.Selection(selection=[('pre_fond1','رئيس قطاع التمويلات'),('pre_fond2','انتهاء التحليل')])
    recommandation_pre_fond  = fields.Html(string='رأي رئيس قطاع التمويلات')
    # President Finance 
    state_pre_fin = fields.Selection(selection=[('pre_fin1','رئيس قطاع المالية'),('pre_fin2','انتهاء التحليل')])
    recommandation_pre_fin = fields.Html(string='رأي رئيس قطاع المالية')
    ...
    def create(self, vals):
        ...
        if (res.demande == self.env.ref('dept_wk.type_demande_1') or res.demande == self.env.ref('dept_wk.type_demande_2') or res.demande ==self.env.ref('dept_wk.type_demande_3') or res.demande ==self.env.ref('dept_wk.type_demande_4')) and not res.workflow.workflow_old:
    ...
    def import_data_excel(self):
        ...
        file_stream = BytesIO(base64.b64decode(self.excel_file))
        wb = openpyxl.load_workbook(file_stream, data_only=True)
        sheet = wb.active  # Assuming the first sheet
        vals={}
        actif_record= self.env['import.ocr.actif'].create(vals)
        passif_record= self.env['import.ocr.passif'].create(vals)
        tcr_record= self.env['import.ocr.tcr'].create(vals)
        actif1_record= self.env['import.ocr.actif'].create(vals)
        passif1_record= self.env['import.ocr.passif'].create(vals)
        tcr1_record= self.env['import.ocr.tcr'].create(vals)
        def create_passif_line(sequence,col):
             selected_rubrique=self.env['import.ocr.config'].search([('type','=','passif'),('sequence','=',sequence)])
             passif_id = self.env['import.ocr.passif.line'].create({'montant_n':get_value(col,2),'montant_n1':get_value(col,3)})
             passif_id.rubrique += selected_rubrique
             passif1_id= self.env['import.ocr.passif.line'].create({'montant_n':get_value(col,4),'montant_n1':get_value(col,5)})
             passif1_id.rubrique += selected_rubrique
             passif_record.passif_lines += passif_id
             passif1_record.passif_lines += passif1_id
        
        
        def create_actif_line(sequence,col):
             selected_rubrique=self.env['import.ocr.config'].search([('type','=','actif'),('sequence','=',sequence)])
             actif_id = self.env['import.ocr.actif.line'].create({'montant_n':get_value(col,2),'montant_n1':get_value(col,3)})
             actif_id.rubrique += selected_rubrique
             actif1_id= self.env['import.ocr.actif.line'].create({'montant_n':get_value(col,4),'montant_n1':get_value(col,5)})
             actif1_id.rubrique += selected_rubrique
             actif_record.actif_lines += actif_id
             actif1_record.actif_lines += actif1_id
        
        def create_tcr_line(sequence,col):
             selected_rubrique=self.env['import.ocr.config'].search([('type','=','tcr'),('sequence','=',sequence)])
             tcr_id = self.env['import.ocr.tcr.line'].create({'montant_n':get_value(col,2),'montant_n1':get_value(col,3)})
             tcr_id.rubrique += selected_rubrique
             tcr1_id= self.env['import.ocr.tcr.line'].create({'montant_n':get_value(col,4),'montant_n1':get_value(col,5)})
             tcr1_id.rubrique += selected_rubrique
             tcr_record.tcr_lines += tcr_id
             tcr1_record.tcr_lines += tcr1_id
        ...
        for rec in self:
            ...
            # empreinte et dettes financiere 12
            # rec.passif_id.passif_lines.filtered(lambda r: r.rubrique.sequence == 12).montant_n = get_value(28,1) # year 4
            # rec.passif_id.passif_lines.filtered(lambda r: r.rubrique.sequence == 12).montant_n1 = get_value(28,2) # year 3
            # rec.passif1_id.passif_lines.filtered(lambda r: r.rubrique.sequence == 12).montant_n = get_value(28,3) # year 2
            # rec.passif1_id.passif_lines.filtered(lambda r: r.rubrique.sequence == 12).montant_n1 = get_value(28,4) # year 1
            
            create_passif_line(12,28)
            
            # prime et reserves 4
            # rec.passif_id.passif_lines.filtered(lambda r: r.rubrique.sequence == 4).montant_n = get_value(26,1) # year 4
            # rec.passif_id.passif_lines.filtered(lambda r: r.rubrique.sequence == 4).montant_n1 = get_value(26,2) # year 3
            # rec.passif1_id.passif_lines.filtered(lambda r: r.rubrique.sequence == 4).montant_n = get_value(26,3) # year 2
            # rec.passif1_id.passif_lines.filtered(lambda r: r.rubrique.sequence == 4).montant_n1 = get_value(26,4) # year 1
            
            create_passif_line(4,26)
            
            # autre capiteaux propres 8
            # rec.passif_id.passif_lines.filtered(lambda r: r.rubrique.sequence == 8).montant_n = get_value(27,1) # year 4
            # rec.passif_id.passif_lines.filtered(lambda r: r.rubrique.sequence == 8).montant_n1 = get_value(27,2) # year 3
            # rec.passif1_id.passif_lines.filtered(lambda r: r.rubrique.sequence == 8).montant_n = get_value(27,3) # year 2
            # rec.passif1_id.passif_lines.filtered(lambda r: r.rubrique.sequence == 8).montant_n1 = get_value(27,4) # year 1
           
            
            
            # resultat net de l'exercice 50
            # rec.tcr_id.tcr_lines.filtered(lambda r: r.rubrique.sequence == 50).montant_n = get_value(6,1) # year 4
            # rec.tcr_id.tcr_lines.filtered(lambda r: r.rubrique.sequence == 50).montant_n1 = get_value(6,1) # year 3
            # rec.tcr1_id.tcr_lines.filtered(lambda r: r.rubrique.sequence == 50).montant_n = get_value(6,1) # year 2
            # rec.tcr1_id.tcr_lines.filtered(lambda r: r.rubrique.sequence == 50).montant_n1 = get_value(6,4) # year 1
          
            create_tcr_line(50,6)
            #
            # rec.passif_id.passif_lines.filtered(lambda r: r.rubrique.sequence == 25).montant_n = get_value(36,1) # year 4
            # rec.passif_id.passif_lines.filtered(lambda r: r.rubrique.sequence == 25).montant_n1 = get_value(36,2) # year 3
            # rec.passif1_id.passif_lines.filtered(lambda r: r.rubrique.sequence == 25).montant_n = get_value(36,3) # year 2
            # rec.passif1_id.passif_lines.filtered(lambda r: r.rubrique.sequence == 25).montant_n1 = get_value(36,4) # year 1
           
            create_passif_line(25,36)
            ...
            # rec.actif_id.actif_lines.filtered(lambda r: r.rubrique.sequence == 16).montant_n =get_value(16,1) # year 4
            # rec.actif_id.actif_lines.filtered(lambda r: r.rubrique.sequence == 16).montant_n1=get_value(16,2) # year 3
            # rec.actif1_id.actif_lines.filtered(lambda r: r.rubrique.sequence == 16).montant_n=get_value(16,3) # year 2
            # rec.actif1_id.actif_lines.filtered(lambda r: r.rubrique.sequence == 16).montant_n1=get_value(16,4) # year 1
           
            create_actif_line(16,16)
            
            # rec.actif_id.actif_lines.filtered(lambda r: r.rubrique.sequence == 27).montant_n =get_value(22,1) # year 4
            # rec.actif_id.actif_lines.filtered(lambda r: r.rubrique.sequence == 27).montant_n1=get_value(22,2) # year 3
            # rec.actif1_id.actif_lines.filtered(lambda r: r.rubrique.sequence == 27).montant_n=get_value(22,3) # year 2
            # rec.actif1_id.actif_lines.filtered(lambda r: r.rubrique.sequence == 27).montant_n1=get_value(22,4) # year 1
           
            create_actif_line(27,22)

            # rec.passif_id.passif_lines.filtered(lambda r: r.rubrique.sequence == 24).montant_n = get_value(35,1) # year 4
            # rec.passif_id.passif_lines.filtered(lambda r: r.rubrique.sequence == 24).montant_n1 = get_value(35,2) # year 3
            # rec.passif1_id.passif_lines.filtered(lambda r: r.rubrique.sequence == 24).montant_n = get_value(35,3) # year 2
            # rec.passif1_id.passif_lines.filtered(lambda r: r.rubrique.sequence == 24).montant_n1 = get_value(35,4) # year 1
          
            create_passif_line(24,35)
            ...
            create_passif_line(18,30)

            # rec.passif_id.passif_lines.filtered(lambda r: r.rubrique.sequence == 23).montant_n = get_value(34,1) # year 4
            # rec.passif_id.passif_lines.filtered(lambda r: r.rubrique.sequence == 23).montant_n1 = get_value(34,2) # year 3
            # rec.passif1_id.passif_lines.filtered(lambda r: r.rubrique.sequence == 23).montant_n = get_value(34,3) # year 2
            # rec.passif1_id.passif_lines.filtered(lambda r: r.rubrique.sequence == 23).montant_n1 = get_value(34,4) # year 1
             

            create_passif_line(23,34) 
             
            # rec.actif_id.actif_lines.filtered(lambda r: r.rubrique.sequence == 18).montant_n = get_value(17,1) # year 4
            # rec.actif_id.actif_lines.filtered(lambda r: r.rubrique.sequence == 18).montant_n1 = get_value(17,2) # year 3
            # rec.actif1_id.actif_lines.filtered(lambda r: r.rubrique.sequence == 18).montant_n = get_value(17,3) # year 2
            # rec.actif1_id.actif_lines.filtered(lambda r: r.rubrique.sequence == 18).montant_n1 = get_value(17,4) # year 1
            
            create_actif_line(18,17)
            
            # rec.passif_id.passif_lines.filtered(lambda r: r.rubrique.sequence == 20).montant_n = get_value(31,1) # year 4
            # rec.passif_id.passif_lines.filtered(lambda r: r.rubrique.sequence == 20).montant_n1 = get_value(31,2) # year 3
            # rec.passif1_id.passif_lines.filtered(lambda r: r.rubrique.sequence == 20).montant_n = get_value(31,3) # year 2
            # rec.passif1_id.passif_lines.filtered(lambda r: r.rubrique.sequence == 20).montant_n1 = get_value(31,4) # year 1
           
            create_passif_line(20,31)
            
            # rec.actif_id.actif_lines.filtered(lambda r: r.rubrique.sequence == 20).montant_n = get_value(18,1) # year 4
            # rec.actif_id.actif_lines.filtered(lambda r: r.rubrique.sequence == 20).montant_n1 = get_value(18,2) # year 3
            # rec.actif1_id.actif_lines.filtered(lambda r: r.rubrique.sequence == 20).montant_n = get_value(18,3) # year 2
            # rec.actif1_id.actif_lines.filtered(lambda r: r.rubrique.sequence == 20).montant_n1 = get_value(18,4) # year 1
          
            create_actif_line(20,18)
            
            # rec.actif_id.actif_lines.filtered(lambda r: r.rubrique.sequence == 26).montant_n = get_value(21,1) # year 4
            # rec.actif_id.actif_lines.filtered(lambda r: r.rubrique.sequence == 26).montant_n1 = get_value(21,2) # year 3
            # rec.actif1_id.actif_lines.filtered(lambda r: r.rubrique.sequence == 26).montant_n = get_value(21,3) # year 2
            # rec.actif1_id.actif_lines.filtered(lambda r: r.rubrique.sequence == 26).montant_n1 = get_value(21,4) # year 1
          
            create_actif_line(26,21)
            
            # rec.actif_id.actif_lines.filtered(lambda r: r.rubrique.sequence == 19).montant_n = get_value(19,1) # year 4
            # rec.actif_id.actif_lines.filtered(lambda r: r.rubrique.sequence == 19).montant_n1 = get_value(19,2) # year 3
            # rec.actif1_id.actif_lines.filtered(lambda r: r.rubrique.sequence == 19).montant_n = get_value(19,3) # year 2
            # rec.actif1_id.actif_lines.filtered(lambda r: r.rubrique.sequence == 19).montant_n1 = get_value(19,4) # year 1
          
            create_actif_line(19,19)
            
            # rec.actif_id.actif_lines.filtered(lambda r: r.rubrique.sequence == 24).montant_n = get_value(20,1) # year 4
            # rec.actif_id.actif_lines.filtered(lambda r: r.rubrique.sequence == 24).montant_n1 = get_value(20,2) # year 3
            # rec.actif1_id.actif_lines.filtered(lambda r: r.rubrique.sequence == 24).montant_n = get_value(20,3) # year 2
            # rec.actif1_id.actif_lines.filtered(lambda r: r.rubrique.sequence == 24).montant_n1 = get_value(20,4) # year 1
            
            create_actif_line(24,20)
            
            # rec.passif_id.passif_lines.filtered(lambda r: r.rubrique.sequence == 18).montant_n = get_value(35,1) # year 4
            # rec.passif_id.passif_lines.filtered(lambda r: r.rubrique.sequence == 18).montant_n1 = get_value(35,2) # year 3
            # rec.passif1_id.passif_lines.filtered(lambda r: r.rubrique.sequence == 18).montant_n = get_value(35,3) # year 2
            # rec.passif1_id.passif_lines.filtered(lambda r: r.rubrique.sequence == 18).montant_n1 = get_value(35,4) # year 1
            
            create_passif_line(2,25)
            # rec.passif_id.passif_lines.filtered(lambda r: r.rubrique.sequence == 14).montant_n = get_value(29,1) # year 4
            # rec.passif_id.passif_lines.filtered(lambda r: r.rubrique.sequence == 14).montant_n1 = get_value(29,2) # year 3
            # rec.passif1_id.passif_lines.filtered(lambda r: r.rubrique.sequence == 14).montant_n = get_value(29,3) # year 2
            # rec.passif1_id.passif_lines.filtered(lambda r: r.rubrique.sequence == 14).montant_n1 = get_value(29,4) # year 1
            
            create_passif_line(14,29)
            
            # rec.passif_id.passif_lines.filtered(lambda r: r.rubrique.sequence == 21).montant_n = get_value(32,1) # year 4
            # rec.passif_id.passif_lines.filtered(lambda r: r.rubrique.sequence == 21).montant_n1 = get_value(32,2) # year 3
            # rec.passif1_id.passif_lines.filtered(lambda r: r.rubrique.sequence == 21).montant_n = get_value(32,3) # year 2
            # rec.passif1_id.passif_lines.filtered(lambda r: r.rubrique.sequence == 21).montant_n1 = get_value(32,4) # year 1
            
            create_passif_line(21,32)
            
            # rec.passif_id.passif_lines.filtered(lambda r: r.rubrique.sequence == 22).montant_n = get_value(33,1) # year 4
            # rec.passif_id.passif_lines.filtered(lambda r: r.rubrique.sequence == 22).montant_n1 = get_value(33,2) # year 3
            # rec.passif1_id.passif_lines.filtered(lambda r: r.rubrique.sequence == 22).montant_n = get_value(33,3) # year 2
            # rec.passif1_id.passif_lines.filtered(lambda r: r.rubrique.sequence == 22).montant_n1 = get_value(33,4) # year 1
            
            create_passif_line(22,33)
            
            # rec.tcr_id.tcr_lines.filtered(lambda r: r.rubrique.sequence == 36).montant_n = get_value(7,1) # year 4
            # rec.tcr_id.tcr_lines.filtered(lambda r: r.rubrique.sequence == 36).montant_n1 = get_value(7,2) # year 3
            # rec.tcr1_id.tcr_lines.filtered(lambda r: r.rubrique.sequence == 36).montant_n = get_value(7,3) # year 2
            # rec.tcr1_id.tcr_lines.filtered(lambda r: r.rubrique.sequence == 36).montant_n1 = get_value(7,4) # year 1
            
            create_tcr_line(36,7)
            
            # rec.tcr_id.tcr_lines.filtered(lambda r: r.rubrique.sequence == 12).montant_n = get_value(10,1) # year 4
            # rec.tcr_id.tcr_lines.filtered(lambda r: r.rubrique.sequence == 12).montant_n1 = get_value(10,2) # year 3
            # rec.tcr1_id.tcr_lines.filtered(lambda r: r.rubrique.sequence == 12).montant_n = get_value(10,3) # year 2
            # rec.tcr1_id.tcr_lines.filtered(lambda r: r.rubrique.sequence == 12).montant_n1 = get_value(10,4) # year 1
            
            create_tcr_line(12,10)
            
            # rec.tcr_id.tcr_lines.filtered(lambda r: r.rubrique.sequence == 13).montant_n = get_value(8,1) # year 4
            # rec.tcr_id.tcr_lines.filtered(lambda r: r.rubrique.sequence == 13).montant_n1 = get_value(8,2) # year 3
            # rec.tcr1_id.tcr_lines.filtered(lambda r: r.rubrique.sequence == 13).montant_n = get_value(8,3) # year 2
            # rec.tcr1_id.tcr_lines.filtered(lambda r: r.rubrique.sequence == 13).montant_n1 = get_value(8,4) # year 1
            
            create_tcr_line(13,8)
            
            # rec.tcr_id.tcr_lines.filtered(lambda r: r.rubrique.sequence == 14).montant_n = get_value(9,1) # year 4
            # rec.tcr_id.tcr_lines.filtered(lambda r: r.rubrique.sequence == 14).montant_n1 = get_value(9,2) # year 3
            # rec.tcr1_id.tcr_lines.filtered(lambda r: r.rubrique.sequence == 14).montant_n = get_value(9,3) # year 2
            # rec.tcr1_id.tcr_lines.filtered(lambda r: r.rubrique.sequence == 14).montant_n1 = get_value(9,4) # year 1
            
            create_tcr_line(14,9)
            
            create_tcr_line(7,4)
            create_tcr_line(33,5)
            
            
            
            
        self.tcr_id = tcr_record
        self.actif_id = actif_record
        self.passif_id = passif_record
        self.tcr1_id =tcr1_record
        self.passif1_id=passif1_record
        self.actif1_id=actif1_record
        self.actif_id.state="valide"
        self.tcr_id.state="valide"
        self.passif_id.state="valide"
        
       



    def open_companies_wizard(self):
        domain_ids = []   
        
        workflows_state_7 = self.env['res.partner'].search([
                            ("workflows_linked", '=', False)
                        ])
        domain_ids = [(6, 0, workflows_state_7.ids)]    
        return {
            'name': 'Confirmation',
            'type': 'ir.actions.act_window',
            'res_model': 'wk.companies.wizard',
            'view_mode': 'form',
            'target': 'new',
            'context': {'default_company_domains': self.id,
                        'default_client_name_domain':domain_ids 
                        }
        }
        ...
        for rec in self:
            ...
            if rec.etape.sequence == 1:
                ...
                if rec.state_tres == 'tres_1':
                    ...
                    rec.state_compute = 1
            elif rec.etape.sequence == 16:
                if rec.state_pre_fond == 'pre_fond1':
                    rec.state_compute = 0
                elif rec.state_pre_fond == 'pre_fond2':
                    rec.state_compute = 1
            elif rec.etape.sequence == 17:
                if rec.state_pre_fin == 'pre_fin1':
                    rec.state_compute = 0
                elif rec.state_pre_fin == 'pre_fin2':
                    rec.state_compute = 1                            
            elif rec.etape.sequence == 5:
    ...
    def validate_information_function(self):
        for rec in self:
            if rec.etape.sequence == 1:
                if rec.state_branch == 'branch_1':
                    ...
                    if self.env.user.has_group('dept_wk.dept_wk_group_responsable_agence'):
                        ...
                        if not etape_revision:
                            ...
                            etape_revision.companies.unlink()
                ...
                if rec.state_risque == 'risque_1':
                    ...
                    if self.env.user.has_group('dept_wk.dept_wk_group_responsable_risque'):
                        ...
                        rec.workflow.state = '16'
                        etape_1 = rec.workflow.states.filtered(lambda l: l.etape.sequence == 1)
                        etape_comm = rec.workflow.states.filtered(lambda l: l.etape.sequence == 3)
                        etape_risk = rec.workflow.states.filtered(lambda l: l.etape.sequence == 4)
                        etape_fin = rec.workflow.states.filtered(lambda l: l.etape.sequence == 10)
                        etape = rec.workflow.states.filtered(lambda l: l.etape.sequence == 16)
                        ...
                        vals = {
                            ...
                            'num_compte': etape_1.num_compte,
                            'demande': etape_1.demande.id,
                            'state_pre_fond': 'pre_fond1',
                            'etape': self.env.ref('dept_wk.princip_16').id,
                            'gerant': etape_1.gerant.id,
                            'unit_prod': etape_1.unit_prod,
                            'stock': etape_1.stock,
                            'prod_company': etape_1.prod_company,
                ...
                if rec.state_pre_fond == 'pre_fond1':
                    etape_fin = etape_2 = rec.workflow.states.filtered(lambda l: l.etape.sequence == 2)
                    rec.state_pre_fond = 'pre_fond2'
                    rec.raison_a_revoir = False
                    if etape_2.comite == self.env.ref('dept_wk.pouvoir_1'):

                        
                        # etape_fin.state_finance = 'finance_4'
                        etape_fin.raison_a_revoir = False
                        rec.workflow.state = '17'
                        etape_1 = rec.workflow.states.filtered(lambda l: l.etape.sequence == 1)
                        etape_comm = rec.workflow.states.filtered(lambda l: l.etape.sequence == 3)
                        etape_risk = rec.workflow.states.filtered(lambda l: l.etape.sequence == 4)
                        etape_fin = rec.workflow.states.filtered(lambda l: l.etape.sequence == 10)
                        etape = rec.workflow.states.filtered(lambda l: l.etape.sequence == 17)
                        ...
                        vals = {
                            ...
                            'num_compte': etape_1.num_compte,
                            'demande': etape_1.demande.id,
                            'state_pre_fin': 'pre_fin1',
                            'etape': self.env.ref('dept_wk.princip_17').id,
                            'gerant': etape_1.gerant.id,
                            'unit_prod': etape_1.unit_prod,
                            'stock': etape_1.stock,
                            'prod_company': etape_1.prod_company,
            ...
            self.update_state_finance_related()
    ...
    def import_data_group(self):
        for rec in self:
            if rec.tcr_group.state not in ['valide', 'modified'] or rec.actif_group.state not in ['valide', 'modified'] or rec.passif_group.state not in ['valide', 'modified']:
                ...
                # مجموع الديون
                passif_2 = rec.passif_group.passif_lines.filtered(lambda r: r.rubrique.sequence == 18)
                passif_3 = rec.passif_group.passif_lines.filtered(lambda r: r.rubrique.sequence == 24)
    ...
    def import_data(self):
        for rec in self:
            if rec.tcr_id.state not in ['valide', 'modified'] or rec.actif_id.state not in ['valide', 'modified'] or rec.passif_id.state not in ['valide', 'modified']:
                ...
                elif passif_1.montant_n1 < 0:
                    bilan_18.write({
                                'year_4': (passif_18.montant_n + passif_24.montant_n) / passif_1.montant_n if passif_1.montant_n != 0 else 0,
                                'year_3': 'N/A',
                                'year_2': (passif1_18.montant_n + passif1_24.montant_n) / passif1_1.montant_n if passif1_1.montant_n != 0 else 0,
                                'year_1': (passif1_18.montant_n1 + passif1_24.montant_n1) / passif1_1.montant_n1 if passif1_1.montant_n1 != 0 else 0,
                                })
                elif passif1_1.montant_n < 0: 
                    bilan_18.write({
                                'year_4': (passif_18.montant_n + passif_24.montant_n) / passif_1.montant_n if passif_1.montant_n != 0 else 0,
                                'year_3': (passif_18.montant_n1 + passif_24.montant_n1) / passif_1.montant_n1 if passif_1.montant_n1 != 0 else 0,
                                'year_2': 'N/A',
        ...
        for seq in range(1, 30):  # Loop from bilan_1 to bilan_33
            current_bilan = rec.bilan_id.filtered(lambda r: r.sequence == seq)
            if current_bilan:  # Ensure bilan exists
                current_bilan.write({
                    'year_4': current_bilan.year_4 / 1000 if current_bilan.year_4 else 0,
                    'year_3': current_bilan.year_3 / 1000 if current_bilan.year_3 else 0,
                    'year_2': current_bilan.year_2 / 1000 if current_bilan.year_2 else 0,
                    'year_1': current_bilan.year_1 / 1000 if current_bilan.year_1 else 0,
                })       
    ...
    def authenticate(self):
        ...
        url = 'https://10.116.70.27:9443/irf-auth-token-generation-container-22.0.99/api/v1.0.0/generateauthtoken'
        try:
            _logger.info('Sending authentication request to %s', url)
            response = requests.get(url, auth=("ASBA.AGENT", "Ws*7410+"), verify=False)
            _logger.info('Response received: %s', response.status_code)
            if response.status_code == 200:
                _logger.info('Authentication successful')
                parsed_data = response.json()
                if isinstance(parsed_data, str):
                    parsed_data = json.loads(parsed_data)
                token = parsed_data.get('id_token')
                if not token:
                    raise UserError(_("No 'id_token' found in response: %s") % parsed_data)
            ...
            raise UserError(_("An error occurred while fetching data: %s") % str(e))
        except Exception as e:
            raise UserError(_("An unexpected error occurred: %s") % str(e))

    customer_id = fields.Integer(string='Customer ID', related='nom_client.id_customer')
    cient_accounts = fields.One2many('wk.compte.client', 'etape_id', string="Client Accounts")
    
    def get_customer_accounts(self):
        for rec in self:
            token = self.authenticate()
            url = f"https://10.116.70.27:9443/asbaws/api/v1.0.0/holdings/accounts/{rec.customer_id}"
            _logger.info("Calling get_customer_info for customer_id=%s", rec.customer_id)
            try:
                headers = {
                ...
                _logger.info("Holdings API response status: %s", response.status_code)
                if response.status_code != 200:
                    _logger.error('Failed to fetch client data: %s', response.status_code)
                    raise UserError(_("Failed to fetch client data. Server responded with status code %s") % response.status_code)
                data = response.json()
                if "body" not in data or not isinstance(data["body"], list) or not data["body"]:
                    _logger.warning('No account data found for account number: %s', rec.customer_id)
                    raise UserError(_("No account data found for the given account number."))
                account_data = data["body"][0]
                consulter_compte_wizard = self.env['wk.consulter.compte'].create({'etape_id': self.id})
                compte_records = []
                for account_data in data["body"]:
                    new_compte = self.env['wk.compte.client'].create({
                        'etape_id': consulter_compte_wizard.id,
                        'account_id': account_data.get('accountId', ''),
                        'ledgerBalance': account_data.get('ledgerBalance', 0.0),
                        'workingBalance': account_data.get('workingBalance', 0.0),
                        'accountCurrency': account_data.get('accountCurrency', ''),
                        'categoryName': account_data.get('categoryName', ''),
                    })
                    compte_records.append(new_compte.id)
                if compte_records:
                    consulter_compte_wizard.write({'compte_client_ids': [(6, 0, compte_records)]})
            except requests.exceptions.RequestException as e:
                raise UserError(_("An error occurred while fetching data: %s") % str(e))
            except Exception as e:
                raise UserError(_("An unexpected error occurred: %s") % str(e))
            return {
                'type': 'ir.actions.act_window',
                'name': 'Consulter Liste des Comptes',
                'res_model': 'wk.consulter.compte',
                'view_mode': 'form',
                'res_id': consulter_compte_wizard.id,
                'target': 'new',
            }
