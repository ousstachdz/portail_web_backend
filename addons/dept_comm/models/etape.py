...
list_recommandation = [('موافق', 'موافق'),('غير موافق', 'غير موافق')]
...
class Etape(models.Model):
    ...
    recommendation_1 = fields.Text(string='تعليق مدير إدارة الاعمال التجارية')
    recommendation_1_decision = fields.Selection(string='توصية مدير إدارة الاعمال التجارية',selection=list_recommandation)
    ...
    excel_file_ponctuel = fields.Binary(string='الملف الاكسيل')
    has_old_workflow = fields.Boolean(
        string="Has Old Workflow",
        compute="_compute_has_old_workflow",
        store=False
    )

    @api.depends()
    def _compute_has_old_workflow(self):
        for record in self:
            record.has_old_workflow = bool(record.workflow.workflow_old_ponctuel or record.workflow.workflow_old_dashboard)
    ...
    def action_create_actif(self):
        for rec in self:
            ...
            return {
                'name': 'Actif',
                'domain': [('step_id', '=', rec.id)],
                'res_model': 'import.ocr.actif.ponctuel',
                'view_mode': 'form',
                'view_id': view_id,
                'type': 'ir.actions.act_window',
                'context': {'step_id': rec.id}
    ...
    def action_create_passif(self):
        for rec in self:
            ...
            return {
                'name': 'Passif',
                'domain': [('step_id', '=', rec.id)],
                'res_model': 'import.ocr.passif.ponctuel',
                'view_mode': 'form',
                'view_id': view_id,
                'type': 'ir.actions.act_window',
                'context': {'step_id': rec.id}
    ...
    def action_create_actif_1(self):
        for rec in self:
            view_id = self.env.ref('financial_modeling.import_ocr_actif_view_form').id
            return {
                'name': 'Actif',
    ...
    def action_create_passif_1(self):
        for rec in self:
            view_id = self.env.ref('financial_modeling.import_ocr_passif_view_form').id
            return {
                'name': 'Passif',
    ...
    def import_data_excel_ponctuel(self):
         # Open the Excel file using openpyxl
       
        
        file_stream = BytesIO(base64.b64decode(self.excel_file_ponctuel))
