...
list_type_fin_demande = [('طور تبليغ المتعامل', 'طور تبليغ المتعامل'),('المدير العام', 'المدير العام')]
...
class Ponctuel(models.Model):
    ...
    workflow_old_type=fields.Selection([('ponctuel',' طلب التمويل المؤقت'),('dashboard','طلب التمويل')],string='نوع ملف سابق',default='dashboard')
    workflow_old = fields.Many2one('wk.workflow.dashboard', string='ملف سابق')
    workflow_old_ponctuel= fields.Many2one('wk.workflow.ponctuel',string='ملف سابق')
    workflow_old_dashboard= fields.Many2one('wk.workflow.dashboard',string='ملف سابق')
    ...
    recommendation_1 = fields.Text(string='تعليق')
    recommendation_1_decision = fields.Selection(string='توصية مدير إدارة الاعمال التجارية',selection=list_recommandation)
    
    recommendation_2 = fields.Text(string='تعليق')
    recommendation_2_decision = fields.Selection(string='توصية مدير  إدارة  الدراسات الائتمانية للمؤسسات',selection=list_recommandation)
    
    recommendation_3 = fields.Text(string='تعليق')
    recommendation_3_decision = fields.Selection(string='توصية رئيس  خلية إدارة المخاطر',selection=list_recommandation)
    
    recommendation_4 = fields.Text(string='تعليق')
    recommendation_4_decision = fields.Selection(string='توصية رئيس قطاع  الخزينة و العمليات المحلية و الدولية',selection=list_recommandation)
    
    recommendation_5 = fields.Text(string='تعليق')
    recommendation_5_decision = fields.Selection(string='توصية رئيس خلية إدارة التمويلات',selection=list_recommandation)
    
    recommendation_6 = fields.Text(string='تعليق')
    recommendation_6_decision = fields.Selection(string='توصية مستشار نائب المدير العام المكلف بالاستشراف التجاري',selection=list_recommandation)

    recommendation_7 = fields.Text(string='تعليق')
    recommendation_7_decision = fields.Selection(string='توصية رئيس خلية التحصيل الودي و الجبري',selection=list_recommandation)
    
    recommendation_8 = fields.Text(string='تعليق')
    recommendation_8_decision = fields.Selection(string='توصية نائب المدير العام',selection=list_recommandation)
    
    recommendation_9 = fields.Text(string='تعليق')
    recommendation_9_decision = fields.Selection(string='توصية المدير العام ',selection=list_recommandation)
    
    type_fin_demande=fields.Selection(string='نهاية الطلب', selection = list_type_fin_demande)
    
    def _compute_record_selection(self):
        """Fetch records from Model A and Model B and return them as selection choices."""
       
        records_b = self.env['wk.workflow.dashboard'].search([])
        
        print('records_b', records_b)
        selection_list = []

        for rec in records_b:
            selection_list.append((f"B-{rec.id}", rec.name))  # Prefix B-
    ...
    def action_start(self):
        if(self.workflow_old_type=="dashboard"):
            workflow_old = self.workflow_old_dashboard
        elif(self.workflow_old_type=="ponctuel"):
            workflow_old = self.workflow_old_ponctuel
        ...
        for rec in self:
            if self.workflow_old_dashboard or self.workflow_old_ponctuel:
               new_risk_scoring = workflow_old.risk_scoring.copy(default={
                    'partner_id': rec.nom_client.id,
                    'ponctuel_id': rec.id,
            })
               new_risk_scoring.write({
                    'etape_id': [(5, 0, 0)]  # Remove all linked records
            })
               rec.risk_scoring = new_risk_scoring.id 
            print("show me this and that ",workflow_old.states)
            states = workflow_old.states.filtered(lambda l: l.sequence in [1, 2])
            states_ids = workflow_old.states
    ...
    solde = fields.Float(string="رصيد الحساب الجاري")
    ...
    def action_get_solde(self, token):
        self.ensure_one()
        if not self.num_compte:
            _logger.warning('Numero Compte is not set before fetching client data')
            raise UserError(_("Please set the Numero Compte before fetching client data."))
        url = f"https://10.116.70.27:9443/asbaws/api/v1.0.0/holdings/balance/{self.num_compte}"
        headers = {
            'Content-Type': 'application/json',
            'Authorization': f'Bearer {token}'
        }
        try:
            _logger.info('Fetching client data from %s', url)
            ...
            if "body" not in data or not isinstance(data["body"], list) or not data["body"]:
                _logger.warning('No account data found for account number: %s', self.num_compte)
                raise UserError(_("No account data found for the given account number."))
            account_data = data["body"][0]
            working_balance = account_data.get("workingBalance", 0.0)
            self.write({
            'solde': working_balance,
            })
            
        except requests.exceptions.RequestException as e:
            _logger.exception('An error occurred while fetching client data')
            raise UserError(_("An error occurred while fetching data: %s") % str(e))
        except Exception as e:
            _logger.exception('An unexpected error occurred while fetching client data')
            raise UserError(_("An unexpected error occurred: %s") % str(e))
    ...
    def validate_information_function(self):
        ...
        for rec in self:
            ...
            if current_state in state_path:
                if current_state == '8' and rec.comite_type == "comite_1" and not rec.type_fin_demande:
                    raise UserError('لم يتم تحديد نهاية الطلب')
                if current_state == '8' and rec.type_fin_demande == 'المدير العام' and rec.comite_type == "comite_1":
                   state_path = ['1', '2', '3', '5', '6', '8', '9' , '10']                    
    ...
    if etape_old.sequence == 1:
        ...
        for doc in etape_old.detail_garantie_actuel_ids:
            self.env['wk.detail.garantie'].create({'type_garantie': doc.type_garantie.id,
                  ...
                  'montant': doc.montant,
                  'montant_dollar': doc.montant_dollar,
                  'date': doc.date,
